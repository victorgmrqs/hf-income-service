...
| REC-04 | Receitas recorrentes são propagadas para a próxima competência via `POST /income/propagate` | ✅ | Job executado no 1º dia do mês (scheduler interno via goroutine, `src/internal/scheduler`) cria cópia com `origin_id` apontando para a receita original — HF-98 (endpoint) + HF-38 (disparo automático) |
