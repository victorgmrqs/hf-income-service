...
| ORC-05 | Auto-ajuste ocorre no 1º dia do mês via job (`POST /budgets/global/auto-adjust`, idempotente); teto é sempre editável manualmente | ✅ | Edição manual sobrescreve o auto-ajuste e define `auto_adjusted: false`; disparo automático via scheduler interno (goroutine), não K8s CronJob — HF-44 (endpoint) + HF-38 (disparo automático) |
