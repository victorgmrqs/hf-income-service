...
### 1.3 Regras de negócio no Notion
Acesse o Notion via MCP. Leia a página de regras do(s) domínio(s) identificado(s) no ticket.
Workspace: https://www.notion.so/30ee65ad4ba98086809ed3a3f38ef45f
...
dod_checklist:
  ...
  - "[ ] Notion atualizado (se regra adicionada/modificada)"
...
Documente a nova regra no Notion via MCP antes de implementar o código que a referencia.
...
Consulte `.claude/skills/testing-guide.md` para padrões de setup.
...
| Condição | Ação |
|----------|------|
| Nova regra | Adicionar linha em `docs/rules/<DOMAIN>.md` + Notion via MCP |
| Regra modificada | Atualizar linha em `docs/rules/<DOMAIN>.md` + Notion via MCP |
