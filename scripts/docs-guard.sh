...
# 1. Mudança em internal/ exige entrada no CHANGELOG.
if has '^internal/' && ! has '^CHANGELOG\.md$'; then
  err "código em internal/ mudou sem entrada no CHANGELOG.md ([Unreleased])."
fi
...
# 3. Código de produção em internal/ exige um teste (*_test.go) no mesmo diff.
PROD=$(echo "$CHANGED" | grep -E '^internal/.*\.go$' | grep -vE '_test\.go$' || true)
if [ -n "$PROD" ] && ! echo "$CHANGED" | grep -qE '_test\.go$'; then
  err "código em internal/ mudou sem nenhum teste (*_test.go) no mesmo diff."
fi

# 4. Mudança de contrato REST exige openapi.yaml.
if has '^internal/handler/' && ! has '^openapi\.ya?ml$'; then
