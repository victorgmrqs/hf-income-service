...
// MET-04: combinação (user_id, category_id, competence) é única **entre registros
// ativos** — índice único PARCIAL (WHERE deleted_at IS NULL), criado em
// repository.EnsureReductionGoalIndexes fora do AutoMigrate (mesmo padrão do
// ADR-001/GlobalBudget: a tag uniqueIndex do GORM não expressa índice parcial, e
// um índice full impediria recriar a meta após um soft delete).
